...
main.prototype = Object.create(Object.prototype) <<< do
  ...
  handler: (t) ->
    ...
    @que.map (n) ~>
      ...
      for k,v of vs =>
        v = if main.intptr[tp[k]] => that(vs[k], ve[k], percent) else ve[k]
  ...
  animate: (n, ve, dur = 333) ->
    ...
    for k, v of ve =>
      ...
      for t in main.intorder =>
        if !ret = main.intparser[t](vs[k], ve[k]) => continue
        tp[k] = t
        break
    a = {vs, ve, tp, dur, st: 0}
...
if window? => window.transition = main
